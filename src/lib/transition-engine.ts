...
export class TransitionEngine {
  ...
  static beforeNavigate = async (nav: { from: URL, to: URL, cancel: () => void }) => {
    ...
    goto(nav.to.href).then(() => {
      ...
      fadedElements.forEach(elem => elem.element.style.opacity = elem.opacity)
    })
  }
  ...
  private static _setTransitionElementsPart = (elements: NodeListOf<Element>, part: 'from' | 'to') => {
    elements.forEach(element => {
      const transitionId = element.attributes.getNamedItem(this.transitionAttributeName)?.value
      if(!transitionId) {
        console.warn(`No ID value was found on element tagged by '${this.transitionAttributeName}' attribute.`)
        return
      }
  ...
  private static _cloneElement = (source: HTMLElement) => {
    ...
    clone.style.right   = right + scrollY - padding.right - padding.left - margin.right - margin.left + 'px'
    clone.style.bottom  = bottom + scrollX - padding.top - padding.bottom - margin.top - margin.bottom + 'px'
  ...
  private static _setTransitionElements = () => {
    ...
    const destinationElements = this._destinationFrame.element.contentWindow?.document?.body.querySelectorAll(`[${this.transitionAttributeName}]`)
  ...
  private static _applyTransitions = async () => {
    ...
    await new Promise(resolve => {
      setTimeout(() => {
        ...
        resolve(undefined)
      }, this.transitionDuration + this.fadeDuration)
    })
    return faded
  }
  ...
  private static _fadeOutSource = () => {
    ...
    for (let i = 0; i < document.body.children.length; i++) {
      ...
      element.animate([
        ...
        duration: this.fadeDuration > this.transitionDuration ? this.transitionDuration : this.fadeDuration,
        fill: 'forwards'
      })
      .commitStyles()
    }
  ...
  private static _fadeInDestination = async () => {
    this._destinationFrame.element.animate([
      { opacity: '0', zIndex: '9999' },
      { opacity: '1', zIndex: '9999' }
    ], { duration: this.fadeDuration, fill: 'forwards' }).commitStyles()
